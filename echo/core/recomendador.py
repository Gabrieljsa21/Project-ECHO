...
def calcular_score(candidato, perfil):
    ...
    if historico_mod.foi_recomendada_recentemente(candidato["titulo"], candidato["artista"]):
        return -1.0, None
    if _artista_rejeitado(candidato, perfil) or historico_mod.artista_tem_feedback_negativo(candidato["artista"]):
        return -1.0, None
    ...
    return sum(componentes.values()), categoria_dominante


def ranquear(candidatos, perfil):
    """Só os candidatos com score > 0, ordenados do maior pro menor, cada um com
    `_score`/`_categoria` anexados pro `core/radar.py` montar a seleção."""
    ranqueados = []
    for candidato in candidatos:
        score, categoria = calcular_score(candidato, perfil)
        if score > 0:
