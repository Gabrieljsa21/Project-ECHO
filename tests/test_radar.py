...
def _perfil_boom_bap():
    ...
    return perfil_mod.carregar_perfil()
...
def test_gerar_10_recomendacoes_com_candidatos_suficientes():
    ...
    radar = radar_mod.gerar_radar(candidatos, quantidade=10)
    assert len(radar) == 10
...
def test_impedir_duplicatas_entre_edicoes():
    ...
    primeiro_radar = radar_mod.gerar_radar(candidatos, quantidade=10)
    ids_primeiro = {c["titulo"] for c in primeiro_radar}

    segundo_radar = radar_mod.gerar_radar(candidatos, quantidade=10)
...
def test_nao_preencher_quota_com_candidato_de_score_baixo():
    perfil_mod.definir_peso_genero("jazz experimental", 0.0)
